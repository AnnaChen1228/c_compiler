...
class Parser:
    ...
    def statement(self):
        ...
        if self.checkToken(TokenType.PRINT):
            ...
            if self.checkToken(TokenType.STRING):
                ...
                self.nextToken()
            elif self.checkToken(TokenType.MAX) or self.checkToken(TokenType.MIN):
                checkmax=False
                if self.checkToken(TokenType.MAX):
                    checkmax=True
                else:
                    checkmax=False
                self.nextToken()
                a=self.curToken.text
                self.nextToken()
                b=self.curToken.text
                nummax=0
                nummin=0
                if a>b:
                    nummax=a
            ...
            elif self.checkToken(TokenType.SORT):
                #print('sort')
                self.nextToken()
                num=[]
                while self.checkToken(TokenType.NUMBER):
                    #print(int(self.curToken.text))
                    num.append(int(self.curToken.text))
                    self.nextToken()
                newnum=sorted(num)
                #print(newnum)
                self.emitter.emitLine('printf("'+str(newnum)+'\\n");')
            else:
        ...
        # "INPUT" ident
        ...
        elif self.checkToken(TokenType.MAX):
            self.nextToken()
            #print(self.curToken)
            #self.nextToken()
            #print(self.curToken)
            #if(self.curToken==TokenType.NUMBER and self.nextToken()==TokenType.NUMBER):
            #print("min")
            a=self.curToken.text
            self.nextToken()
            b=self.curToken.text
            self.emitter.emitLine("max(" + a + ","+b+")")
            self.match(TokenType.NUMBER)
            ...
            while self.checkToken(TokenType.NUMBER):
                ...
                num.append(self.curToken.text)
                self.nextToken()
            newnum=sorted(num)
            self.emitter.emitLine(str(newnum))
    ...
    def primary(self):
        if self.checkToken(TokenType.NUMBER): 
            self.emitter.emit(self.curToken.text)#number
            a=self.curToken.text
            #print(a)
            if self.checkPeek(TokenType.ASTERISK):
                self.nextToken()
                #print(1)
                self.emitter.emit(self.curToken.text)#*
                #print(self.peekToken.text)
                if self.checkPeek(TokenType.ASTERISK):
                    self.nextToken()
                    #print("pow")
                    #print(a)
                    self.emitter.emit(a)#number
            self.nextToken()
